...
echo "--- 2. Выполнение миграций, создание суперюзера и сбор статики ---"
# Используем python3 вместо python для явного указания интерпретатора.
python3 manage.py collectstatic --noinput
python3 manage.py migrate

# --- Создание суперпользователя неинтерактивно ---
# Требует DJANGO_SUPERUSER_USERNAME, DJANGO_SUPERUSER_EMAIL и DJANGO_SUPERUSER_PASSWORD
if [ -n "$DJANGO_SUPERUSER_USERNAME" ] && [ -n "$DJANGO_SUPERUSER_PASSWORD" ]; then
    echo "Создание суперпользователя..."
    # Используем DJANGO_SUPERUSER_PASSWORD для установки пароля.
    echo "from users.models import User; User.objects.create_superuser('$DJANGO_SUPERUSER_USERNAME', '$DJANGO_SUPERUSER_EMAIL', '$DJANGO_SUPERUSER_PASSWORD')" | python3 manage.py shell || true
else
    echo "Переменные DJANGO_SUPERUSER_* не установлены. Суперпользователь не будет создан."
fi
# --- КОНЕЦ БЛОКА ---
